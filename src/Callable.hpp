...
class Callable<R, ne, true, Args...> : public virtual _Object<nullptr> {
    public:
    virtual R operator()(Args...) const noexcept(ne) = 0;
};

template <class R, bool ne>
class Callable<R, ne, true, void> : public virtual _Object<nullptr> {
    public:
    virtual R operator()(void) const noexcept(ne) = 0;
};

template <class R, bool ne, class... Args>
class Callable<R, ne, false, Args...> : public virtual _Object<nullptr> {
    public:
    virtual R operator()(Args...) noexcept(ne) = 0;
};

template <class R, bool ne>
class Callable<R, ne, false, void> : public virtual _Object<nullptr> {
    public:
